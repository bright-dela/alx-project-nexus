...
class ProductAdmin(admin.ModelAdmin):
    ...
    def average_rating(self, obj):
        ...
        if avg:
            avg_value = float(avg)
            stars = "★" * int(avg_value) + "☆" * (5 - int(avg_value))
...
class ProductImageAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Image", {"fields": ("image", "image_preview", "alt_text")}),
        ("Display", {"fields": ("is_primary", "display_order")}),
    ...
    def image_preview(self, obj):
        if obj.image:
            return format_html(
                '<img src="{}" style="max-width: 500px;" />', obj.image.url
            )
        return "No image"
...
class ProductReviewAdmin(admin.ModelAdmin):
    ...
    def rating_display(self, obj):
        ...
        return format_html(
            '<span style="color: #f39c12; font-size: 16px;">{}</span>', stars
        )
